...
int validar(char *p_numero, int *r){
    ...
    if(size == 1 && *(p_numero) == ',') // Si el usuario ingresa simplemente una coma, no es un número válido.
        retorno = 1;
    else if(*(p_numero+size-1) == ',') // Si el usuario ingresa un número que termina con una coma, no es un número válido.
        retorno = 1;
    else {
        for(i = 0; i < size && retorno == 0; i++){
            int igual;
            igual = 1;
            if(*(p_numero+i) == ',') { // Si el caracter es una coma, la cuenta ya que si contiene más de 1, es inválido.
                    contador++;
                    igual = 0;
            } else {
